...
import { DashboardSkeleton } from "../../../../../components/workspace/admin/dashboard/DashboardSkeleton.jsx";
import { EmptyState } from "../../../../../components/workspace/admin/dashboard/EmptyState.jsx";
import { KpiCard } from "../../../../../components/workspace/admin/dashboard/KpiCard.jsx";
import { getActivityIcon } from "../../../../../components/workspace/admin/dashboard/getActivityIcon.jsx";
import { getActivityColor } from "../../../../../components/workspace/admin/dashboard/getActivityColor.js";
import { ClockCalendarCard } from "../../../../../components/share/clock-calendar-card.jsx";
...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async (showRefreshToast = false) => {
    ...
    try {
      ...
      const leaveData =
        leaveRequestsRes.status === "fulfilled" && leaveRequestsRes.value?.data
          ? leaveRequestsRes.value.data
          : [];
      setLeaveRequests(leaveData);

      const pendingLeaveRequests = leaveData.filter(
        (l) => l.status === "pending_pm" || l.status === "pending_gm",
      ).length;

      // Process Payroll
      const payrollData =
        payrollRes.status === "fulfilled" && payrollRes.value?.data
      ...
      // Process Policies
      const policiesData =
        policiesRes.status === "fulfilled" && policiesRes.value?.data
      ...
      const projectChartData = Object.values(projectStatusMap).filter(
        (item) => item.value > 0,
      );
      ...
      const leaveChartData = Object.values(leaveStatusMap).filter(
        (item) => item.value > 0,
      );
              ...
              <CardContent className="space-y-4">
                {recentActivities.length > 0 ? (
                  recentActivities.map((activity) => (
                    ...
                    >
                      <div
                        className={`p-2 rounded-lg bg-${getActivityColor(activity.type)}-50 dark:bg-${getActivityColor(activity.type)}-950/30`}
                      >
