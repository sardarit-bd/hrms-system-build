      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto p-4 sm:p-6">
        <DialogHeader>
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <DialogTitle className="text-base sm:text-xl">{member.full_name}</DialogTitle>
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm">{member.email}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm">{member.phone || "—"}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="p-3 rounded-lg bg-gray-50 dark:bg-gray-900/50">
              {shift === "Not assigned" ? (
                <p className="text-xs sm:text-sm text-gray-500">No shift assigned</p>
              ) : (
                <div className="space-y-1">
                  <p className="text-sm font-medium">{shift.name}</p>
                  <p className="text-xs text-gray-500">Timing: {shift.timing}</p>
                  <p className="text-xs text-gray-500">Weekend: {shift.weekend}</p>
                  <Badge className={member.roster?.is_active ? "bg-green-100 text-green-700 text-[10px] sm:text-xs" : "bg-gray-100 text-gray-700 text-[10px] sm:text-xs"}>
          ...
          <div>
            ...
            <div className="p-3 rounded-lg bg-gray-50 dark:bg-gray-900/50">
              {!policy ? (
                <p className="text-xs sm:text-sm text-gray-500">No policy assigned</p>
              ) : (
                <div className="space-y-1">
                  ...
                  <Badge className={member.policy?.is_active ? "bg-green-100 text-green-700 text-[10px] sm:text-xs" : "bg-gray-100 text-gray-700 text-[10px] sm:text-xs"}>
