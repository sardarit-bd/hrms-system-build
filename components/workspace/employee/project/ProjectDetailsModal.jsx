...
import HourLogsTable from "./HourLogsTable";
...
export default function ProjectDetailsModal({ open, onOpenChange, project }) {
  ...
  const [projectHourLogs, setProjectHourLogs] = useState([]);
  const [activeTab, setActiveTab] = useState("details");

  useEffect(() => {
    if (project && open && project.type === "hourly") {
      fetchProjectHourLogs();
    }
  }, [project, open]);

  const fetchProjectHourLogs = async () => {
    if (!project) return;
    setLoading(true);
    try {
      const response = await apiRequest(`/hour-logs/project/${project.id}`);
      if (response.status && response.data) {
        setProjectHourLogs(response.data);
      }
    } catch (error) {
      console.error("Failed to fetch project hour logs:", error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex gap-4 border-b pb-2">
          ...
          {project.type === "hourly" && (
            ...
            >
              Hour Logs ({projectHourLogs.length})
            </button>
          )}
        </div>
