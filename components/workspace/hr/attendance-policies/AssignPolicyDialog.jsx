...
export default function AssignPolicyDialog({ open, onOpenChange, policies, employees, onSuccess }) {
  ...
  const handleSubmit = async () => {
    if (!formData.user_id || !formData.attendance_policy_id) {
      gooeyToast.error("Missing Fields", { description: "Please select employee and policy." });
      return;
    }
    ...
    try {
      ...
      if (response.status) {
        gooeyToast.success("Policy Assigned", { description: "Attendance policy has been assigned successfully." });
        onOpenChange(false);
        setFormData({
          user_id: "",
          attendance_policy_id: "",
