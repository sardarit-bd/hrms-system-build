...
import { MoreHorizontal, Calendar, Plus, History } from "lucide-react";

export default function RosterAssignments({
  rosters = [],
  employees = [],
  shifts = [],
  onAssignRoster,
  ...
  if (rosters.length === 0) {
    return (
      <div className="rounded-lg border border-gray-200 dark:border-gray-800 p-8 sm:p-12 text-center">
        ...
        <Button
          onClick={onAssignRoster}
          className="mt-2 bg-[#1D3A88] hover:bg-[#142558] cursor-pointer text-sm"
        >
  ...
  const MobileCardView = () => (
    <div className="space-y-3 sm:hidden p-3">
      {rosters.map((roster) => (
        <div
          key={roster.id}
          className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-gray-800 p-3"
        >
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm font-medium">
              {getEmployeeName(roster.user_id)}
            </p>
          ...
          <div className="flex gap-2 mt-3 pt-2 border-t border-gray-100 dark:border-gray-800">
            <Button
              ...
              size="sm"
              onClick={() =>
                onViewHistory(employees.find((e) => e.id === roster.user_id))
              }
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-40">
                    ...
                    <DropdownMenuItem
                      onClick={() =>
                        onViewHistory(
                          employees.find((e) => e.id === roster.user_id)
                        )
                      }
  ...
  return (
    <div className="rounded-lg border border-gray-200 dark:border-gray-800 overflow-hidden bg-white dark:bg-slate-950">
      <div className="flex justify-end p-3 sm:p-4 border-b border-gray-200 dark:border-gray-800">
        <Button
          onClick={onAssignRoster}
          className="bg-[#1D3A88] hover:bg-[#142558] cursor-pointer text-sm"
        >
          <Plus size={14} className="mr-2" />
          Assign Roster
        </Button>
